...
If values such as border width are configured in preview-canvas points, scale or normalize them against the preview canvas before export so the saved output matches what the user saw.
...
- If tap selects but drag/pinch manipulates content, selection should happen on tap only. Starting a pan or pinch should not also trigger selection side effects.
- Keep transient drag/pinch values local to the manipulated subview and commit the final normalized state at gesture end instead of mutating parent view state every frame.
- Put gestures on the manipulable content layer, not on decorative overlays. Selection borders, tints, and badges should usually use `.allowsHitTesting(false)` unless they are intentionally interactive.
